...
class AttackUtils(object):
    ...
    def attack_bs_pgd(self, model, X, y, epsilon, alpha, restarts):
        max_loss = torch.zeros(y.shape[0]).cuda()
        max_delta = torch.zeros_like(X).cuda()
        for zz in range(restarts):
            delta = torch.zeros_like(X).cuda()
    ...
    def evaluate_pgd(self, test_loader, model, attack_iters, restarts, epsilon=8, l2_norm=False, blacksmith=False):
        epsilon = (epsilon / 255.) / self.std
        ...
        for i, (X, y) in enumerate(test_loader):
            X, y = X.cuda(), y.cuda()
            if blacksmith:
                pgd_delta = self.attack_bs_pgd(model, X, y, epsilon, 0.5 * epsilon, restarts)
            elif l2_norm:
